...
const Search = () => {
  ...
  useEffect(() => {
    if (search) {
      fetch(`https://api.themoviedb.org/3/search/movie?api_key=b1666d3d17f247efa7f49e045debdf4a&query=${search}`)
        .then(res => res.json())
      ...
      <div className="bg-gradient-to-r from-purple-500 to-pink-500 min-h-screen pt-12"> {/* Adjusted for navbar height */}
        ...
        <div className="container mx-auto p-4">
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {results.length > 0 ? (
              results.map(movie => (
                <div key={movie.id} className="bg-moviee rounded-lg overflow-hidden shadow-lg p-4">
                  <Link to={`/kk/${movie.id}`}>
